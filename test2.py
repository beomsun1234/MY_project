...
class mycar:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("/usb_cam/image_raw/compressed/compressed", CompressedImage, self.callback)## PARKIN
    ...
    def run(self):
        if self.ck >1550 and self.pix_l> self.pix_r: ##right 0.85
            self.steering = 0.1
            self.motor_msg.data = -1000
            print("go back left1")
        elif self.ck >1550 and self.pix_l<self.pix_r: ##left 0.15
            self.steering = 0.95
            self.motor_msg.data = -1000
            print("go back right2")
        elif self.pix_l < 5 and self.pix_r < 5:
            print("stop3")
