...
# 
    intialTracbarVals = [7,60,0,100] #5,60,0,100]
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            dst_steer_level = 0.5304 - float(deviation)*2.4 #2.0
            fps_str += " calAng: %5.2f" % deviation   
            fps_str += " dstStr: %5.2f" % dst_steer_level   
            
            if lane_error_detect > 0: lane_error_detect-=1
            ctrl_lat.pub_cmd(1*1000, dst_steer_level)
            if abs(old_dst_steer_level - dst_steer_level) > 0.19 and lane_error_detect == 0:
                ck = ck +1
                print(ck)
