...
class Lane_mycar:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("/usb_cam2/image_raw/compressed", CompressedImage, self.callback)
    ...
    def callback(self, msg):
        ...
        self.ck = abs(self.pix_l-self.pix_r) ##check pix
        #print(self.pix_l,self.pix_r)

        if self.ack == 0:  ## ack = 0 go , 1= stop
    ...
    def controll(self):
        if self.ck >2500 and self.pix_l> self.pix_r:
            self.steering = 0.75
            self.motor_msg.data = 1000
            print("go right")
        elif self.ck > 2500 and self.pix_l<self.pix_r:
            self.steering = 0.25
            self.motor_msg.data = 1000
            print("go left")
